...
// ✅ Ensure deleted_projects exists and matches columns
$conn->query("CREATE TABLE IF NOT EXISTS deleted_projects LIKE projects");
$conn->query("ALTER TABLE deleted_projects ADD COLUMN IF NOT EXISTS deleted_at DATETIME DEFAULT CURRENT_TIMESTAMP");

// ✅ Get column names shared by both tables
$colRes = $conn->query("
    SELECT COLUMN_NAME 
...
// Filter only columns that actually exist in both arrays
...
$types = str_repeat('s', count($validCols));
$values = [];
foreach ($validCols as $c) {
    $values[] = (string)$project[$c];
}
...
try {
    ...
    // Related tables for soft delete
    $relatedTables = [
        'project_materials',
        'development_reports',
        'project_units',
    ...
    foreach ($relatedTables as $table) {
        ...
        // Create mirror deleted_ table
        $conn->query("CREATE TABLE IF NOT EXISTS deleted_$table LIKE $table");
        $conn->query("ALTER TABLE deleted_$table ADD COLUMN IF NOT EXISTS deleted_at DATETIME DEFAULT CURRENT_TIMESTAMP");
    ...
    // Remove project from active table
    ...
    echo "<meta http-equiv='refresh' content='2;url=projects.php'>";
    exit();
